...
namespace FlaUI.TestUtilities
{
    ...
    /// </summary>
    public abstract class FlaUITestBase2
    {
        /// <summary>
     ...
     Path.Combine(
        ...
        private void StopVideoRecorder()
        {
          ...
          // 1. Determine the default behavior
          bool shouldKeep = KeepVideoForSuccessfulTests;

         // 2. Check for the attribute on the method or class to override the default
         var testMethod = TestContext.CurrentContext.Test.MethodName;
         var testClass = TestContext.CurrentContext.Test.ClassName;

         // Check Method first, then Class (Method takes precedence)
         var methodInfo = this.GetType().GetMethod(testMethod);
         var attribute = (KeepVideoAttribute)Attribute.GetCustomAttribute(methodInfo, typeof(KeepVideoAttribute)) 
                    ?? (KeepVideoAttribute)Attribute.GetCustomAttribute(this.GetType(), typeof(KeepVideoAttribute));

        if (attribute != null)
        {
           shouldKeep = attribute.Keep;
        }

       // 3. Logic for deletion
       if (!shouldKeep && TestContext.CurrentContext.Result.FailCount == 0)
       {
